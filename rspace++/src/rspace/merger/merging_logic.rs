...
pub fn depends(target: &EventLogIndex, source: &EventLogIndex) -> bool {
    let produces_source: HashSet<Produce> = produces_created_and_not_destroyed(source)
        .difference(&source.produces_mergeable)
        .cloned()
        .collect();

    let produces_target: HashSet<Produce> = target
        .produces_consumed
        .difference(&source.produces_mergeable)
        .cloned()
        .collect();
    ...
    let produces_depends: HashSet<Produce> = produces_source
        .intersection(&produces_target)
        .cloned()
        .collect();

    let consumes_depends: HashSet<Consume> = consumes_source
...
pub fn conflicts(a: &EventLogIndex, b: &EventLogIndex) -> Vec<Blake2b256Hash> {
    ...
    let races_for_same_io_event = {
        let shared_consumes: HashSet<Consume> = a
            .consumes_produced
            .intersection(&b.consumes_produced)
            .cloned()
            .collect();
        let mergeable_consumes: HashSet<Consume> = a
            .consumes_mergeable
        ...
        let shared_produces: HashSet<Produce> = a
            .produces_consumed
            .intersection(&b.produces_consumed)
            .cloned()
            .collect();
        let mergeable_produces: HashSet<Produce> = a
            .produces_mergeable
...
pub fn produces_created(e: &EventLogIndex) -> HashSet<Produce> {
...
pub fn consumes_created(e: &EventLogIndex) -> HashSet<Consume> {
...
pub fn produces_created_and_not_destroyed(e: &EventLogIndex) -> HashSet<Produce> {
...
pub fn consumes_created_and_not_destroyed(e: &EventLogIndex) -> HashSet<Consume> {
...
pub fn produces_affected(e: &EventLogIndex) -> HashSet<Produce> {
    let created = produces_created(e);
    let external_produces_destroyed: HashSet<Produce> = e
        .produces_consumed
        .difference(&created)
        .filter(|p| !p.persistent)
        .cloned()
...
pub fn consumes_affected(e: &EventLogIndex) -> HashSet<Consume> {
    let created = consumes_created(e);
    let external_consumes_destroyed: HashSet<Consume> = e
        .consumes_produced
        .difference(&created)
        .filter(|c| !c.persistent)
        .cloned()
...
pub fn combine_produces_copied_by_peek(x: &EventLogIndex, y: &EventLogIndex) -> HashSet<Produce> {
    let combined_copied_by_peek: HashSet<Produce> = x
        .produces_copied_by_peek
        .union(&y.produces_copied_by_peek)
        .cloned()
        .collect();

    let combined_created: HashSet<Produce> = produces_created(x)
...
pub fn compute_relation_map<A: Eq + std::hash::Hash + Clone>(
    ...
    let mut init: HashMap<A, HashSet<A>> = items
        .iter()
...
pub fn gather_related_sets<A: Eq + std::hash::Hash + Clone>(
    relation_map: &HashMap<A, HashSet<A>>,
) -> Vec<HashSet<A>> {
    ...
    for k in relation_map.keys() {
        ...
        if !result.iter().any(|existing_set: &HashSet<A>| {
...
pub fn compute_rejection_options<A: Eq + std::hash::Hash + Clone>(
    ...
    fn gather_rej_options<A: Eq + std::hash::Hash + Clone>(
        conflicts_map: &HashMap<A, HashSet<A>>,
...
mod tests {
    ...
    fn test_compute_rejection_options() {
        ...
        map1.insert(1, HashSet::from_iter(vec![2, 3, 4]));
        map1.insert(2, HashSet::from_iter(vec![1]));
        map1.insert(3, HashSet::from_iter(vec![1, 2]));
        map1.insert(4, HashSet::from_iter(vec![1]));
        ...
        map2.insert(1, HashSet::from_iter(vec![2, 3, 4]));
        map2.insert(2, HashSet::from_iter(vec![1, 3, 4]));
        map2.insert(3, HashSet::from_iter(vec![1, 2, 4]));
        map2.insert(4, HashSet::from_iter(vec![1, 2, 3]));
        ...
        // Test 3
        let mut map3: HashMap<i32, HashSet<i32>> = HashMap::new();
        map3.insert(1, HashSet::from_iter(vec![2, 3, 4]));
        map3.insert(2, HashSet::from_iter(vec![1]));
        map3.insert(3, HashSet::from_iter(vec![1, 4]));
        map3.insert(4, HashSet::from_iter(vec![1, 3]));
        ...
        // Test 4
        let mut map4: HashMap<i32, HashSet<i32>> = HashMap::new();
        map4.insert(1, HashSet::new());
        map4.insert(2, HashSet::from_iter(vec![3]));
        map4.insert(3, HashSet::from_iter(vec![2, 4]));
        map4.insert(4, HashSet::from_iter(vec![3]));
        ...
        let mut map5: HashMap<i32, HashSet<i32>> = HashMap::new();
        for i in 1..=1000 {
    ...
    fn test_compute_related_sets() {
        ...
        let items: HashSet<i32> = HashSet::from_iter(vec![1, 2, 3, 4, 5]);
    ...
    fn test_relation_map() {
        ...
        let items: HashSet<i32> = HashSet::from_iter(vec![2, 3, 4, 6, 12]);
