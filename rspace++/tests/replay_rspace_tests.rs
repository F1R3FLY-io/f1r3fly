...
async fn creating_a_comm_event_should_replay_correctly() {
    ...
    let _ = replay_space.rig_and_reset(
        empty_point.root,
        rig_point.log,
        replay_space.replay_data.clone(),
    );
...
async fn picking_n_datums_from_m_waiting_datums_should_replay_correctly() {
    ...
    fn consume_many<F, G>(
        space: &mut (impl ISpace<String, Pattern, String, String>
                  + Tuplespace<String, Pattern, String, String>),
        range: Vec<i32>,
        channels_creator: F,
...
async fn check_replay_data_should_proceed_if_replay_data_is_empty() {
    ...
    let res = replay_space.check_replay_data(replay_space.replay_data.clone());
    assert!(res.is_ok())
}
...
async fn check_replay_data_should_panic_if_replay_data_contains_elements() {
    ...
    let _ = replay_space.rig_and_reset(c.root, c.log, replay_space.replay_data.clone());
    let _res = replay_space.check_replay_data(replay_space.replay_data.clone());
}
...
async fn fixture() -> StateSetup {
    ...
    let replay_rspace: ReplayRSpace<String, Pattern, String, String> =
        ReplayRSpace::apply(history_repo, replay_store, Arc::new(Box::new(StringMatch)));
