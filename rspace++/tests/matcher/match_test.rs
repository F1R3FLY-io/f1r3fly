...
mod tests {
    ...
    use rspace_plus_plus::rspace::matcher::spatial_matcher::{
        SpatialMatcher, SpatialMatcherContext,
    };
    ...
    fn matching_extras_with_free_variable_should_work() {
        ...
        let expected_captures = BTreeMap::from([(
            0,
            new_gint_par(9, Vec::new(), false).prepend_expr(new_gint_expr(7), 0),
        )]);
        assert!(assert_spatial_match(target, pattern, Some(expected_captures)).is_ok());
    }
    ...
    fn matching_sets_should_work_with_wildcard_remainders() {
        ...
        assert!(
            assert_spatial_match(target.clone(), pattern.clone(), Some(BTreeMap::new())).is_ok()
        );
    ...
    fn matching_maps_should_work_with_wildcard_remainders() {
        ...
        let just_wildcard =
            new_emap_par(vec![], vec![], true, Some(new_wildcard_var()), vec![], true);
    ...
    fn matching_maps_should_work_with_var_remainders() {
        ...
        let just_remainder =
            new_emap_par(vec![], vec![], true, Some(new_freevar_var(0)), vec![], true);
        ...
        assert!(
            assert_spatial_match(target.clone(), just_remainder, Some(expected_captures)).is_ok()
        );
    }
    ...
    fn matching_a_complicated_connective_should_work() {
        ...
        let single_factor_par: Par =
            vector_par(Vec::new(), true).with_connectives(vec![single_factor]);
        ...
        let pattern =
            vector_par(Vec::new(), true).with_connectives(vec![capture, prime, alternative]);
    ...
    fn matching_bool_should_work() {
        ...
        assert!(
            assert_spatial_match(success_target, pattern.clone(), Some(BTreeMap::new())).is_ok()
        );
        assert!(assert_spatial_match(fail_target, pattern, None).is_ok());
    }
