...
fn that_bucket_4_is_full(table: &PeerTable<KademliaRPCMock>) {
...
mod tests {
    ...
    mod peer_table_with_1_byte_addresses_and_k_3 {
        ...
        mod when_adding_a_peer_to_an_empty_table {
            ...
            #[test]
            fn should_add_it_to_a_bucket_according_to_its_distance() {
                // given
        ...
        mod when_adding_a_peer_when_that_peer_already_exists_but_with_different_ip {
            ...
            #[test]
            fn should_replace_peer_with_new_entry_the_one_with_new_ip() {
                // given
                let ping = KademliaRPCMock::new(true);
                let table = PeerTable::new(local().key().clone(), Some(3), None, &ping);
                table.update_last_seen(&peer1()).unwrap();
                ...
                table.update_last_seen(&new_peer1).unwrap();
            ...
            #[test]
            fn should_move_peer_to_the_end_of_the_bucket_meaning_its_been_seen_lately() {
                // given
                let ping = KademliaRPCMock::new(true);
                let table = PeerTable::new(local().key().clone(), Some(3), None, &ping);
                table.update_last_seen(&peer2()).unwrap();
                table.update_last_seen(&peer1()).unwrap();
                table.update_last_seen(&peer3()).unwrap();
                assert_eq!(bucket_entries_at(DISTANCE_4, &table), vec![peer2(), peer1(), peer3()]);
                ...
                // then
                assert_eq!(bucket_entries_at(DISTANCE_4, &table), vec![peer2(), peer3(), new_peer1]);
            }
        }
        ...
        mod when_adding_a_peer_to_a_table_where_corresponding_bucket_is_filled_but_not_full {
            ...
            #[test]
            fn should_add_peer_to_the_end_of_the_bucket_meaning_its_been_seen_lately() {
                // given
                let ping = KademliaRPCMock::new(true);
                let table = PeerTable::new(local().key().clone(), Some(3), None, &ping);
                table.update_last_seen(&peer2()).unwrap();
                table.update_last_seen(&peer3()).unwrap();
                assert_eq!(bucket_entries_at(DISTANCE_4, &table), vec![peer2(), peer3()]);
                
                // when
                table.update_last_seen(&peer1()).unwrap();
                
                // then
                assert_eq!(bucket_entries_at(DISTANCE_4, &table), vec![peer2(), peer3(), peer1()]);
            }

            #[test]
            fn no_peers_should_be_pinged() {
                // given
                let ping = KademliaRPCMock::new(true);
                let table = PeerTable::new(local().key().clone(), Some(3), None, &ping);
                table.update_last_seen(&peer2()).unwrap();
        ...
        mod when_adding_a_peer_to_a_table_where_corresponding_bucket_is_full {
            ...
            #[test]
            fn should_ping_the_oldest_peer_to_check_if_it_responds() {
                // given
                let ping = KademliaRPCMock::new(true);
                let table = PeerTable::new(local().key().clone(), Some(3), None, &ping);
                that_bucket_4_is_full(&table);
                
                // when
                table.update_last_seen(&peer4()).unwrap();
            ...
            mod and_oldest_peer_is_not_responding_to_ping {
                ...
                #[test]
                fn should_add_the_new_peer_and_drop_the_oldest_one() {
                    // given
                    ...
                    // then
                    assert_eq!(bucket_entries_at(DISTANCE_4, &table), vec![peer2(), peer3(), peer4()]);
                }
            }
        }
    }
