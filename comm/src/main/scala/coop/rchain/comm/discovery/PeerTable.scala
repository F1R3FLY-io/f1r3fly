...
final case class PeerTableEntry[A: Show](entry: A, gkey: A => Seq[Byte]) extends Keyed {
  var pinging           = false
  val key: Seq[Byte]    = gkey(entry)
  override def toString = s"#{PeerTableEntry ${entry.show}}"
}

object PeerTable {

  def apply[A <: PeerNode: Show, F[_]: Sync: KademliaRPC: Log](
      key: Seq[Byte],
  ...
  implicit def pteShow[A: Show]: Show[PeerTableEntry[A]] = Show.fromToString[PeerTableEntry[A]]
...
final class PeerTable[A <: PeerNode: Show, F[_]: Sync: KademliaRPC: Log](
    localKey: Seq[Byte],
  ...
  def updateLastSeen(peer: A): F[Unit] = {
    ...
    def bucket: F[Option[mutable.ListBuffer[Entry]]] =
      (for {
        dist <- OptionT.fromOption[F](distance(localKey, peer.key))
        far  = dist < (8 * width)
        idx  <- OptionT.fromOption[F](far.guard[Option].as(dist))
        entr <- OptionT.fromOption[F](Try(table(idx)).toOption)
      } yield entr).value
        ...
        .delay {
          ps synchronized {
            ps.find(_.key == peer.key) match {
              case Some(entry) => // There's already an entry for "peer"
                ps -= entry
                ps += new Entry(peer, _.key)
                None
              case None =>
                if (ps.size < k) { // No entry for "peer" and threshold not met
                  ps += new Entry(peer, _.key)
                  None
                } else {
