...
BITCOIN_ANCHOR_FFI_AMD64_BUILD_ARTIFACTS_PATH="target/$AMD64_TARGET/release"
BITCOIN_ANCHOR_FFI_AARCH64_BUILD_ARTIFACTS_PATH="target/$AARCH64_TARGET/release"
...
cross build --release --target $AMD64_TARGET -p bitcoin_anchor_ffi
cross build --release --target $AARCH64_TARGET -p bitcoin_anchor_ffi

cp -r "$BITCOIN_ANCHOR_FFI_AMD64_BUILD_ARTIFACTS_PATH"/libbitcoin_anchor_ffi.* "./$RUST_LIBRARIES_AMD64_RELEASE_PATH"/
cp -r "$BITCOIN_ANCHOR_FFI_AARCH64_BUILD_ARTIFACTS_PATH"/libbitcoin_anchor_ffi.* "./$RUST_LIBRARIES_AARCH64_RELEASE_PATH"/
