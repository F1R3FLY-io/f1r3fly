...
impl Validate {
    ...
    pub async fn block_summary(
        ...
        Self::parents(block, genesis, s, estimator).await?;
    ...
    pub fn repeat_deploy(
        ...
        let init_parents = proto_util::get_parents_metadata(&s.dag, &block_metadata)?;

        // Calculate max block number and earliest acceptable block number
        let max_block_number = proto_util::max_block_number_metadata(init_parents.clone());
      ...
      let duplicated_deploy = duplicated_deploys
        .iter()
        .find(|processed_deploy| deploy_key_set.contains(&processed_deploy.deploy.sig))
        .expect("Duplicated deploy should exist");

      let term = &duplicated_deploy.deploy.data.term;
      let deployer_string = PrettyPrinter::build_string_bytes(&duplicated_deploy.deploy.pk.bytes);
      let timestamp_string = duplicated_deploy.deploy.data.time_stamp.to_string();
      ...
      BlockError::Invalid(InvalidBlock::InvalidRepeatDeploy)
        ...
        maybe_error.map_or(Ok(ValidBlock::Valid), Err)
    }
    ...
    pub fn timestamp(
        ...
        if before_future && after_latest_parent {
            ...
            Err(BlockError::Invalid(InvalidBlock::InvalidTimestamp))
        }
    }

    /// Agnostic of non-parent justifications
    pub fn block_number(b: &BlockMessage, s: &mut CasperSnapshot) -> ValidBlockProcessing {
        let parents: Vec<BlockMetadata> = proto_util::parent_hashes(b)
            .iter()
            .map(|parent_hash| match s.dag.lookup(parent_hash)? {
                Some(parent_metadata) => Ok(parent_metadata),
                None => Err(KvStoreError::KeyNotFound(format!(
        ...
        if result {
            ...
            Err(BlockError::Invalid(InvalidBlock::InvalidBlockNumber))
        }
    }
    ...
    pub fn future_transaction(b: &BlockMessage) -> ValidBlockProcessing {
        ...
        let deploys: Vec<_> = processed_deploys
            .iter()
            .map(|processed_deploy| &processed_deploy.deploy.data)
            .collect();
    ...
    pub fn sequence_number(b: &BlockMessage, s: &mut CasperSnapshot) -> ValidBlockProcessing {
        let creator_justification_seq_number =
            match proto_util::creator_justification_block_message(b) {
                Some(justification) => match s.dag.lookup(&justification.latest_block_hash)? {
                    Some(block_metadata) => block_metadata.sequence_number as i64,
                    None => {
                        return Err(BlockError::BlockException(CasperError::from(
        ...
        if result {
            ...
            log::warn!("{}", Self::ignore(b, &message));
            Err(BlockError::Invalid(InvalidBlock::InvalidSequenceNumber))
        }
    }
    ...
    pub fn shard_identifier(b: &BlockMessage, shard_id: &str) -> ValidBlockProcessing {
        if b.shard_id == shard_id {
            ...
            Err(BlockError::Invalid(InvalidBlock::InvalidShardId))
        }
    }
    ...
    pub fn block_hash(b: &BlockMessage) -> ValidBlockProcessing {
        ...
        if b.block_hash == block_hash_computed {
            ...
            Err(BlockError::Invalid(InvalidBlock::InvalidBlockHash))
        }
    }
    ...
    pub async fn parents(
        ...
        if parent_hashes == computed_parent_hashes {
            ...
            log::warn!("{}", Self::ignore(b, &message));
            Err(BlockError::Invalid(InvalidBlock::InvalidParents))
        }
    }
    ...
    pub fn justification_follows(
        ...
        let main_parent_hash = parent_hashes
            .first()
            .ok_or_else(|| BlockError::Invalid(InvalidBlock::InvalidParents))?;
        ...
        if bonded_validators == justified_validators {
            ...
            log::warn!("{}", Self::ignore(b, &message));
            Err(BlockError::Invalid(InvalidBlock::InvalidFollows))
        }
    }
                ...
                loop {
                    match remaining_lms.as_slice() {
                        ...
                        [new_lm, tail @ ..] => {
                            ...
                            let new_justification = s.dag.lookup_unsafe(new_justification_hash)?;
                            let cur_justification = s.dag.lookup_unsafe(cur_justification_hash)?;
    ...
    pub fn neglected_invalid_block(
        ...
        for justification in &block.justifications {
            let latest_block_opt = s.dag.lookup(&justification.latest_block_hash)?;
        ...
        let result = if neglected_invalid_justification {
            Err(BlockError::Invalid(InvalidBlock::NeglectedInvalidBlock))
        } else {
            Ok(ValidBlock::Valid)
        };
        ...
        match runtime_manager.compute_bonds(&tuplespace_hash).await {
            Ok(computed_bonds) => {
                ...
                if bonds_set == computed_bonds_set {
                    ...
                    Err(BlockError::Invalid(InvalidBlock::InvalidBondsCache))
                }
            }
            Err(ex) => {
                log::warn!("Failed to compute bonds from tuplespace hash: {}", ex);
                Err(BlockError::BlockException(ex))
            }
        }
    }
    ...
    pub fn phlo_price(b: &BlockMessage, min_phlo_price: i64) -> ValidBlockProcessing {
            ...
            .all(|deploy| deploy.deploy.data.phlo_price >= min_phlo_price)
        {
            ...
            Err(BlockError::Invalid(InvalidBlock::LowDeployCost))
        }
    }
}
