...
use models::rhoapi::{
    BindPattern, ListParWithRandom, Par, TaggedContinuation, tagged_continuation::TaggedCont,
};
...
impl<T: ISpace<models::rhoapi::Par, BindPattern, ListParWithRandom, TaggedContinuation> + 'static>
    ChargingRSpace<T>
{
    fn consume(
        &mut self,
    ...
    pub fn new(space: T, cost_manager: CostManager) -> Self {
        ChargingRSpace {
            space,
            cost_manager,
        }
...
fn handle_result(
    ...
    match result {
        Some((cont, data_list)) => {
            ...
            cost_manager.charge(Cost::create(
            ...
            if last_iteration {
                cost_manager.charge(event_storage_cost(triggered_by_channels_count))?;
            }

            cost_manager.charge(comm_event_storage_cost(cont.channels.len() as i64))
        }
        None => cost_manager.charge(event_storage_cost(triggered_by_channels_count)),
    }
}
