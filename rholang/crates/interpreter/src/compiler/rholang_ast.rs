...
impl<'ast> Proc<'ast> {
    pub fn new_var(name: &'ast str, pos: SourcePosition) -> Proc<'ast> {
        Proc::ProcVar(Var::new_id(name, pos))
    }

    pub fn new_list(elements: Vec<AnnProc<'ast>>) -> Proc<'ast> {
    ...
    pub fn new_tuple(elements: Vec<AnnProc<'ast>>) -> Proc<'ast> {
        Proc::Collection(Collection::Tuple { elements })
    }

    pub fn annotate(&'ast self, pos: SourcePosition) -> AnnProc<'ast> {
        AnnProc { proc: self, pos }
    }

    pub fn annotate_dummy(&'ast self) -> AnnProc<'ast> {
        AnnProc {
            proc: self,
    ...
    pub fn quoted(&'ast self) -> Name<'ast> {
        Name::Quote(self)
    }
}
...
impl<'ast> ASTBuilder<'ast> {
    ...
    pub fn alloc_uri_literal(&self, value: Uri<'ast>) -> &Proc<'ast> {
    ...
    pub fn alloc_var_ref(&self, var_ref: VarRef<'ast>) -> &Proc<'ast> {
    ...
    pub fn alloc_send_sync(&self, name: Name<'ast>, messages: Vec<AnnProc<'ast>>) -> &Proc<'ast> {
    ...
    pub fn alloc_new(&self, proc: &'ast Proc, decls: Vec<NameDecl<'ast>>) -> &Proc {
    ...
    pub fn alloc_bundle(&self, bundle_type: BundleType, proc: &'ast Proc) -> &Proc<'ast> {
        self.arena.alloc(Proc::Bundle { bundle_type, proc })
    }

    pub fn alloc_match(&self, expression: AnnProc<'ast>, cases: Vec<Case<'ast>>) -> &Proc<'ast> {
    ...
    pub fn alloc_for(&self, receipts: Vec<Receipt<'ast>>, proc: AnnProc<'ast>) -> &Proc<'ast> {
    ...
    pub fn alloc_eval(&self, name: AnnName<'ast>) -> &Proc {
        self.arena.alloc(Proc::Eval { name })
    }

    pub fn alloc_quote(&self, proc: &'ast Proc) -> &Proc {
        self.arena.alloc(Proc::Quote(proc))
    }

    pub fn alloc_or(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Or { left, right })
    }

    pub fn alloc_and(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::And { left, right })
    }

    pub fn alloc_matches(&self, target: AnnProc<'ast>, pattern: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Matches { target, pattern })
    }

    pub fn alloc_eq(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Eq { left, right })
    }

    pub fn alloc_neq(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Neq { left, right })
    }

    pub fn alloc_lt(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Lt { left, right })
    }

    pub fn alloc_lte(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Lte { left, right })
    }

    pub fn alloc_gt(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Gt { left, right })
    }

    pub fn alloc_gte(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Gte { left, right })
    }

    pub fn alloc_concat(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Concat { left, right })
    }

    pub fn alloc_diff(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Diff { left, right })
    }

    pub fn alloc_sub(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Sub { left, right })
    }

    pub fn alloc_add(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Add { left, right })
    }

    pub fn alloc_interpolation(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Interpolation { left, right })
    }

    pub fn alloc_mult(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Mult { left, right })
    }

    pub fn alloc_div(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Div { left, right })
    }

    pub fn alloc_mod(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Mod { left, right })
    }

    pub fn alloc_conjunction(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Conjunction { left, right })
    }

    pub fn alloc_disjunction(&self, left: AnnProc<'ast>, right: AnnProc<'ast>) -> &Proc {
        self.arena.alloc(Proc::Disjunction { left, right })
    }

    pub fn alloc_not(&self, proc: &'ast Proc) -> &Proc {
        self.arena.alloc(Proc::Not(proc))
    }

    pub fn alloc_neg(&self, proc: &'ast Proc) -> &Proc {
        self.arena.alloc(Proc::Neg(proc))
    }

    pub fn alloc_negation(&self, proc: &'ast Proc) -> &Proc {
        self.arena.alloc(Proc::Negation(proc))
    }

    pub fn alloc_list(&self, procs: Vec<AnnProc<'ast>>) -> &Proc {
    ...
    pub fn alloc_set(&self, procs: Vec<AnnProc<'ast>>) -> &Proc {
    ...
    pub fn alloc_tuple(&self, procs: Vec<AnnProc<'ast>>) -> &Proc {
        self.arena
            .alloc(Proc::Collection(Collection::Tuple { elements: procs }))
    }

    pub fn alloc_map(&self, elements: Vec<KeyValuePair<'ast>>) -> &Proc {
    ...
    pub fn alloc_choice(&self, branches: Vec<Branch<'ast>>) -> &Proc {
        self.arena.alloc(Proc::Choice { branches })
    }

    pub fn get_node_value(&self, node: &Node) -> Result<&'ast str, InterpreterError> {
        node.utf8_text(self.source_code)
            .or_else(|e| Err(InterpreterError::from_utf8_error(&e, node)))
    }

    pub fn fold_procs_into_par<I>(&'ast self, procs: &'ast [Proc], positions: I) -> &'ast Proc<'ast>
    where
