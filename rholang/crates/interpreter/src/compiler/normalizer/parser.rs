...
use crate::{
    compiler::{
        ...
        rholang_ast::{
            ASTBuilder, AnnName, AnnProc, Binding, Branch, BundleType, Case, GFALSE, GTRUE, Id,
            KeyValuePair, LinearBind, NIL, NameDecl, Names, PeekBind, Proc, Receipt, RepeatedBind,
            SendType, Source, TYPE_BOOL, TYPE_BYTEA, TYPE_INT, TYPE_STRING, TYPE_URI, VarRef,
            VarRefKind, WILD,
        },
    },
    ...
    'parse: loop {
        ...
        match step {
            Step::Done => {
                assert!(
                    proc_stack.len() == 1,
                    "interpreter bug: parsing finished prematurely\n.Remaining process stack: {proc_stack:#?}"
                );
...
fn expect_stack_len(n: usize, stack: &[StackItem], cont_stack: &[K]) {
    ...
    assert!(
        n <= len,
        "interpreter bug: process stack underflow!!!\nProcess stack: {stack:#?}\nContinuation stack: {cont_stack:#?}"
    );
}
...
fn panic_no_process(item: &StackItem) -> ! {
    panic!(
        "interpreter bug: expected to find a process on the top of the process stack, but found {item:#?}"
    );
}

fn panic_no_names(item: &StackItem) -> ! {
    panic!(
        "interpreter bug: expected to find names on the top of the process stack, but found {item:#?}"
    );
}

fn panic_no_processes(stack: &[StackItem]) -> ! {
    panic!(
        "interpreter bug: expected to find process(es) on the top of the process stack, but found ... {stack:#?} <--- top"
    )
}

fn panic_no_let(item: &StackItem) -> ! {
    panic!(
        "interpreter bug: expected to find let binding on the top of the process stack, but found {item:#?}"
    );
}

fn panic_no_binder(item: &StackItem) -> ! {
    panic!(
        "interpreter bug: expected to find a binder on the top of the process stack, but found {item:#?}"
    );
}
