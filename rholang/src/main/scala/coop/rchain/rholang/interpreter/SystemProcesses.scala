      ...
      def produceLargeData: Contract[F] = {
        case isContractCall(produce, Seq(ack)) =>
          ...
          val expensivePar = Par(
            ...
            matches = (0 until 1000).map { i =>
              Match(
                target = GInt(i),
                cases = (0 until 1000).map { j =>
                  MatchCase(
                    pattern = Par(
                      exprs = List(
                        Expr(
            ...
            sends = (0 until 1000).map { i =>
              Send(
                chan = GInt(i),
                data = List(
                  Par(
                    exprs = (0 until 1000).map {
                      j =>
                        Expr(
                          exprInstance = EMethodBody(
                            EMethod(
            ...
            connectives = (0 until 1000).map { i =>
              Connective(
                ConnAndBody(
                  ConnectiveBody(
                    ps = List.fill(1000)(
