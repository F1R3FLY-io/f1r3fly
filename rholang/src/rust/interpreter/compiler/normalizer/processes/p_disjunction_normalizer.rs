...
use crate::rust::interpreter::compiler::free_map::{ConnectiveInstance, FreeMap};
...
pub fn normalize_p_disjunction(
    ...
    let connective = if let Some(Connective::ConnOr(conn_body)) = left_par.single_connective_mut() {
        conn_body.push(right_par);
        left_par.cast_to_connective()
    } else {
        Connective::ConnOr(vec![left_par, right_par])
    };
    input_par.push_connective(connective, depth);

    free_map.remember_connective(ConnectiveInstance::Or, pos);
            ...
            test(
                ...
                |actual_par, free_map| {
                    let expected_par = Par {
                        ...
                        connectives: vec![Connective::ConnOr(Par::free_vars(2))],
                        ..Default::default()
                    };
