...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🌐 Explorer: http://localhost:5000/tx/{}", txid_str);
    println!();
    ...
    for (i, output) in tx.vout.iter().enumerate() {
        if output.scriptpubkey_type == "op_return" {
            ...
            if output.scriptpubkey.len() >= 68 { // "6a20" + 64 hex chars
    ...
    println!("🧱 Last Finalized Block:    {}", hex::encode(&original_state.lfb_hash));
    println!("🌿 RSpace State Root:       {}", hex::encode(&original_state.rspace_root));
    println!("📏 Casper Block Height:     {}", original_state.block_height);
    println!("⏰ Finalization Timestamp:  {} ({})", original_state.timestamp, format_timestamp(original_state.timestamp));
    println!("👥 Validator Set Hash:      {}", hex::encode(&original_state.validator_set_hash));
    ...
    println!("💽 Hash in Bitcoin:         {}", hex::encode(&commitment_hash));
    println!("🧮 Computed from data:      {}", hex::encode(&computed_hash));
    ...
    println!("✅ Transaction confirmed in Bitcoin block: {} confirmations", 
             if tx.status.confirmed { tx.status.block_height.map_or("pending".to_string(), |_| "confirmed".to_string()) } 
