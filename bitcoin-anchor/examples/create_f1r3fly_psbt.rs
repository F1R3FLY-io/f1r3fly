use bitcoin_anchor::bitcoin::{EsploraClient, AnchorPsbt};
use bitcoin_anchor::bitcoin::RetryConfig;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 2 {
        eprintln!("Usage: cargo run --example create_f1r3fly_psbt -- <testnet_address>");
        eprintln!("Example: cargo run --example create_f1r3fly_psbt -- tb1q76a3ee8zr78443xf9v64lcfpanvqwp2apmclrz");
        std::process::exit(1);
    }
    ...
    let address = match Address::from_str(address_str) {
        Ok(addr) => {
            if !addr.is_valid_for_network(bitcoin::Network::Testnet) {
                eprintln!("❌ Address is not valid for Bitcoin testnet");
                std::process::exit(1);
            }
            addr.assume_checked()
        }
    ...
    println!("✅ Address validated for testnet");
    println!();
    ...
    println!("✅ Found {} UTXOs totaling {} sats", utxos.len(), total_input);
    ...
    let psbt_constructor = AnchorPsbt::with_esplora(Network::Testnet3, client);
    
    let fee_rate = Some(10.0); // 10 sats/vbyte - reasonable for testnet
    let change_address = address.clone(); // Send change back to same address
    
    let psbt_transaction = psbt_constructor.build_psbt_transaction_from_address(
        &state,
        &address,
        &change_address,
        fee_rate,
    ...
    println!("   Commitment output index: {}", psbt_transaction.commitment_output_index);
    println!();

    // Step 4: Export PSBT to file
    println!("💾 Exporting PSBT...");
    let psbt_filename = format!("f1r3fly_transaction_{}.psbt", 
                               &address_str[..8]); // First 8 chars of address
    ...
    println!("   🔗 Base64 preview: {}...", &psbt_base64[..60.min(psbt_base64.len())]);
    println!();
