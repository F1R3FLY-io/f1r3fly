...
/// Ports Scala's: property("DER converter check with valid and non-empty input")
#[cfg(test)]
mod der_converter_tests {
    ...
    proptest! {
        ...
        fn test_der_converter_with_valid_non_empty_input(bytes in prop::collection::vec(any::<u8>(), 1..1000)) {
            // Only test with non-empty input (like Scala's if (!bytes.isEmpty))
            if !bytes.is_empty() {
                // Generate a new key pair (equivalent to Secp256k1.newKeyPair)
                let secp256k1 = Secp256k1;
                let (private_key, _public_key) = secp256k1.new_key_pair();
                
                // Hash the input data (equivalent to Blake2b256.hash(bytes))
                let data = Blake2b256::hash(bytes.clone());
                
                // Sign the data using ETH style (equivalent to Secp256k1Eth.sign(data, privKey))
                ...
                if !sig_rs.is_empty() && sig_rs.len() == 64 {
                    // Encode to DER format (equivalent to CertificateHelper.encodeSignatureRStoDER(sigRS).get)
                    let sig_der = CertificateHelper::encode_signature_rs_to_der(&sig_rs)
                        .expect("DER encoding should succeed for valid signature");
                    
                    // Decode back to R,S format (equivalent to CertificateHelper.decodeSignatureDERtoRS(sigDER).get)
                ...
                match encoded_bytes {
                    ...
                    Err(_) => {
                        ...
                        // (Scala version expects it to always succeed, but Rust is more strict)
                    }
                }
    ...
    /// Ports Scala's: property("encoder should throw exception on empty input")
    #[test]
    fn test_encoder_fails_on_empty_input() {
        ...
        assert!(error_msg.contains("empty"), "Error should mention empty input");
    }

    /// Test: decoder should throw exception on empty input  
    /// Ports Scala's: property("decoder should throw exception on empty input")
    #[test]
    ...
    fn test_known_signature_roundtrip() {
        ...
        assert!(der_result.is_ok(), "DER encoding should succeed for valid 64-byte signature");
        ...
        assert!(rs_result.is_ok(), "DER decoding should succeed for valid DER");

        let decoded_rs = rs_result.unwrap();
        assert_eq!(decoded_rs.len(), 64, "Decoded signature should be exactly 64 bytes");
        ...
        assert_eq!(decoded_rs, test_signature, "Roundtrip should preserve signature data");
    }
    ...
    fn test_encoder_edge_cases() {
        ...
        assert!(result.is_ok(), "Encoder should handle oversized input by using first 64 bytes");
